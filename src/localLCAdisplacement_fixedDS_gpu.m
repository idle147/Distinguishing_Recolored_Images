...
%Uses a diamond search method ala Zhu & Ma 200

%OM @ MISL, om82@drexel.edu 26 Feb 2015

%!OM 11 Dec 2015 - updated to utilize GPU for correlation coef.
%calculation. Reduces comp time by about factor of 2 for u = 5
...
function [Dhat, niter] = localLCAdisplacement_fixedDS_gpu(I,iRC,iCC,C,hW,u,delta)
...
Iref_win = gpuArray(double(I(jnds_RefWin,inds_RefWin,iRC)));
...
%Get windowed data from image
Icomp_win = gpuArray(double(I(jnds_CompWin,inds_CompWin,iCC)));
...
%%
C2 = gpuArray(zeros(1,length(SDSP)));
